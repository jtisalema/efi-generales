...
import {defaultEstadoSiniestro, ESTADOS_DOC_VALUES, ESTADOS_PORTAL_LISTA, ESTADOS_SINI_VALUES} from "./estados_edicion";
...
const SiniestroPortal = ({open, setOpen, aseguradoras, findRamos, setTitulares, setPacientes, ...props}) => {
    ...
    const changeEstadoCliente = ({values, setFieldValue, submitForm}) => {
        ...
        if (isUser && [ESTADOS_SINI_VALUES.porRegularizar, ESTADOS_SINI_VALUES.documentoAdicional].includes(values.estadoPortal)) {
            ...
            props.setComentario(defaultEstadoSiniestro(ESTADOS_SINI_VALUES.porRevisar, user, 'Modificación de siniestro por parte de usuario'));//estado inicial de siniestro
        }
        changeSubdocumentsState({values})
            .finally(submitForm);
    }
        ...
        values?.gastos?.forEach(gasto => {
            ...
            gasto?.subdocumentosItems?.forEach(subdocumentos => {
                subdocumentos?.forEach(subdocumento => {
                    const estructura = subdocumentosEstructura.find(estructura =>
                        estructura.cdDocSiniestro === subdocumento.cdDocSiniestro &&
                        estructura.cdSdocSiniestro === subdocumento.cdSdocSiniestro
                    );
                    if (estructura && !estructura.obligatorio && !subdocumento.pathFile) cdsArchivos.push(subdocumento.cdArchivo);
                });
            });
        });
        if (!cdsArchivos.length) return;
        return axios.post(routes.api + "/documentos-digitales/update/estado", {values: cdsArchivos}, {
            params: {
    ...
    const onRetryEmail = (setAction) => {
        setAction(true);
        axios.post(routesVam.api + '/siniestros-portal/retry-email', props?.item).then(() => {
    ...
    return (<Pages title={"Siniestros"} key={params?.cdReclamo}>
            <section className="p-1 p-md-2 p-xl-4 flex-grow-1">
                <div className="container-fluid">
                    <Alerts ref={alert}/>
                    <UqaiFormik initialValues={props?.item} onSubmit={onSubmit} enableReinitialize={true}
                        ...
                        {({values}) => (
                            <>
                                ...
                                <CardIncapSiniestro viewCard={viewCard} setViewCard={setViewCard}
                                                    aseguradoras={aseguradoras} findRamos={findRamos}
                                                    setPacientes={setPacientes}
                                                    open={open} setOpen={setOpen} setTitulares={setTitulares} {...props}
                                />
                                {!open &&
                                    <>
                                        <div className="row">
                                                            ...
                                                            <CardBody>
                                                                <div className="row">
                                                                    <div className="col-lg-6">
                                                                        ...
                                                                        <span>{values?.numSiniestro ? props?.polizaData?.aseguradora : findList(aseguradoras, Number(values?.cdAseguradora))}</span>
                                                                    </div>
                                                                    <div className="col-lg-6">
                                                                        <label
                                                                            className="form-label fw-bold text-secondary fs-7">Póliza: </label><br/>
                                                                        <span>{values?.numSiniestro ? `${props?.polizaData?.ramo} / ${values?.poliza} / ${props?.polizaData?.plan}` : findPoliza(props.ramos, values)}</span>
                                                                    </div>
                                                                    <div className="col-lg-6">
                                                                        <label
                                                    ...
                                                    {values.documentos &&
                                                        <div className="card w-100 btn-enviar-siniestro">
                                                            <div className="card-header">
                                                                <div
                                                                    className={"d-flex gap-3 justify-content-start align-porps?.items-end"}>
                                                                    {showBtnEstado ?
